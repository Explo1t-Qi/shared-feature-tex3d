...
`docs/up-local-gradient-diagnostic.md` is complete, including its real OpenVLA run
and independent saved-array audit. It used only the original 12 development /
calibration frames and did not rerun screening, publish a neuron set, construct a
shared direction, or start a scope experiment. A frozen-direction follow-up remains
discussion only and is not authorized. The earlier action-relevant protocol below
